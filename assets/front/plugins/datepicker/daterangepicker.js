(function(root, factory) {
  ...
  DateRangePicker.prototype = {
    ...
    renderCalendar: function(side) {
      ...
      html += '<thead>';
      html += '<tr class="monthbar">';
      if (this.showWeekNumbers || this.showISOWeekNumbers)
        html += '<th></th>';
      if ((!minDate || minDate.isBefore(calendar.firstDay)) && (!this.linkedCalendars || side == 'left')) {
        html += '<th class="prev available nav"><span></span></th>'
      } else {
      ...
      if ((!maxDate || maxDate.isAfter(calendar.lastDay)) && (!this.linkedCalendars || side == 'right' || this.singleDatePicker)) {
        html += '<th class="nav next available"><span></span></th>'
      } else {
        html += '<th class="nav"></th>'
      }
      html += '</tr>';
      ...
      $.each(this.locale.daysOfWeek, function(index, dayOfWeek) {
        html += '<th class="dayname">' + dayOfWeek + '</th>'
      });
      html += '</tr>';
      html += '</thead>';
      html += '<tbody>';
      ...
      for (var row = 0; row < 6; row++) {
        ...
        for (var col = 0; col < 7; col++) {
          ...
          if (!disabled)
            cname += 'available';
          var c ="";
          if(calendar[row][col].format('M') == calendar[1][1].format('M')){
            var mm = calendar[row][col],
                year = mm.format('YYYY'),
                month = mm.format('M'),
                cm = moment([year,month-1]),
                endMonth = moment(cm).endOf('month');
            //console.log(calendar[row][col].format('D MM YYYY')+" - "+endMonth.format('D MM YYYY'));
            //console.log(endMonth.format('D MM YYYY') == calendar[row][col].format('D MM YYYY'));
            //console.log("=======================");
            if(endMonth.format('D MM YYYY') == calendar[row][col].format('D MM YYYY')){
              c= "endmonth "
            }else if (calendar[row][col].format('D') == "1"){
              //console.log(calendar[row][col].format('D')+" - "+endMonth.isSame(calendar[row][col]));
              c= "startmonth "
            }
            html += '<td class="' +c+ cname.replace(/^\s+|\s+$/g, '') + '" data-title="' + 'r' + row + 'c' + col + '"><div class="item ' + cname.replace(/^\s+|\s+$/g, '') + '" data-title="' + 'r' + row + 'c' + col + '">' + calendar[row][col].date() + '</div></td>'
          }else{
            html +='<td class="empty" data-title="' + 'r' + row + 'c' + col + '"><div class="empty"></div></td>';
          }
    ...
    clickDate: function(e) {
      ...
      this.updateView();
      this.element.trigger('select.daterangepicker', this);
      var obj = this;
      setTimeout(function(){obj.element.trigger('changes.daterangepicker', obj)},500)
      e.stopPropagation()
    },
