                                            ...
                                            <tr>
                                                <td style="word-break:break-word;font-size:0px;padding:0px 0px 20px;" align="left">
                                                    <div style="cursor:auto;color:#737F8D;font-family:Whitney, Helvetica Neue, Helvetica, Arial, Lucida Grande, sans-serif;font-size:16px;line-height:24px;text-align:left;">
                                                        ...
                                                        <?php $total = $this->query->get_data('total', 'transaksi', ['kode_transaksi' => $order_id])->row()->total; ?>
                                                        <p>Transaksi kamu dengan dengan No.Invoice <b><?= $order_id ?> </b> telah Berhasil dengan total pembayaran <b>Rp <?= rupiah($total) ?></b> </p>
                                                        ...
                                                        <table style="width: 100%;" border="1" cellpadding="5" cellspacing="5">
                                                            ...
                                                            <tbody>
                                                                <?php $detail = $this->query->get_query("SELECT k.judul_kelas,k.link_zoom,k.tgl_kelas,k.waktu_mulai,k.waktu_akhir FROM transaksi t JOIN transaksi_detail d ON t.id = d.transaksi_id JOIN kelas k ON d.product_id = k.id WHERE t.kode_transaksi = '$order_id'")->result();
                                                                foreach ($detail as $d) { ?>
                                                                    <tr>
                                                                        <td><?= $d->judul_kelas ?></td>
                                                                        <td><?= format_indo($d->tgl_kelas) ?></td>
                                                                        <td><?= $d->waktu_mulai ?> - <?= $d->waktu_akhir ?> </td>
                                                                        <!-- <td><a href="<?= $d->link_zoom ?>" class="btn btn-primary" target="_blank">Link</a></td> -->
                                                                        <td><a href="<?= base_url() ?>profile/tiket" style="text-decoration:none;line-height:100%;color:white;font-family:Ubuntu, Helvetica, Arial, sans-serif;font-size:15px;font-weight:normal;text-transform:none;margin:0px;padding:5px;background:#75B248" target="_blank">lihat tiket saya (link zoom)</a></td>
                                                                    </tr>
                                                                <?php } ?>
                                                            </tbody>
                                                        </table>
